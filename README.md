...
1. **Python 3.8+** (runs the backend database server without any external library dependencies)
2. **Git**
...
1. Go to **https://www.python.org/downloads/** and download the latest Python installer for Windows.
2. Run the installer. **Make sure to check the box for "Add python.exe to PATH"** before clicking Install.
3. Complete the installation.
4. Verify Python is installed by opening a new **PowerShell** and running:
   ```powershell
   python --version
   ```
...
```powershell
git clone https://github.com/perryvegehan/Your-OWN-AI.git
cd Your-OWN-AI
```
...
Inside the `Your-OWN-AI` (or cloned folder) directory, run:

```powershell
python app.py
```

This starts the Python HTTP server on port 8080. You should see:
```
=== VectorDB Engine ===
http://localhost:8080
...
| `python: command not found` | Re-run the Python installer and check "Add python.exe to PATH", or add it manually |
...
Then edit [app.py](app.py) line where `self.genModel` is set:
```python
self.genModel = "llama3.2:1b"   # change this
```
